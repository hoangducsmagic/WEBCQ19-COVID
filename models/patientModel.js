...
async function getPatientInfo(patientId) {
	...
	if (statusChangeInfo.length > 0)
		patientInfo.statusChangeList = statusChangeInfo;
	if (facilityChangeInfo.length > 0)
		patientInfo.facilityChangeList = facilityChangeInfo;
...
async function getPatientIdByUsername(username) {
	var query = `
        SELECT pt.patient_id as "patientId"
        FROM patient pt
