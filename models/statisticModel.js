...
async function getDebtList(){
    var debtQuery=`
        ...
        WHERE debt>0
    `
    const debtList=await db.getQuery(debtQuery);
    return debtList;
}

async function getBoughtList(){
  var query=`
    SELECT pt.name as "patientName", pp.name as "packageName", o.total as "total", to_char(o.date,'dd/mm/yyyy') as "date"
    FROM public."order" o
    JOIN productpackage pp on o.productpackage_id=pp.productpackage_id
    JOIN patient pt on o.patient_id=pt.patient_id
  `
  var boughtList=await db.getQuery(query);
  return boughtList;
}
